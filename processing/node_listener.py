...
def main(argv):
    ...
    while True:
        try:
            ...
            if DB_ENABLED:
                sql = "INSERT INTO %s VALUES(NULL, NULL, '%s', '%s', %d, '%s', %d, %d, %d, NULL, NULL, NULL, %d, %d, %d, '%s', %d)" % (DB_TABLE_NAME, nodeID, ip, timestamp, address, channel, counter, rssi, crc, lpe, syncController, label, txPower)
                cursor.execute(sql)
                db.commit()
